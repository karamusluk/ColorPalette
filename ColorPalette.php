 ...
 * (c)
 *
 * This library is a PHP port of the JavaScript library NTC JS with addition to Our Code World's repo.
 *
+-----------------------------------------------------------------+
|     Created by Chirag Mehta  \ enhanced by Mustafa Culban       |
|-----------------------------------------------------------------|
...
specified. Updated with shade color additiion to ease to determine with
basic color naming.
...
class ColorPalette {
	...
	public function __construct() {
		...
		for ($i = 0; $i < count($this->colorsWithShades); $i++) {
			$color = "#" . $this->colorsWithShades[$i][0];
			$rgb = $this->rgb($color);

			$hsl = $this->hsl($color);

			array_push(
				$this->colorsWithShades[$i],
				[
					$rgb[0],
					$rgb[1],
					$rgb[2],
	...
	public function name($color) {
		...
		for ($i = 0; $i < count($this->colorsWithShades); $i++) {
			if ($color == "#" . $this->colorsWithShades[$i][0]) {
				return array(
					"hex" => "#" . $this->colorsWithShades[$i][0],
					"name" => $this->colorsWithShades[$i][1][0],
					"shade_name" => $this->colorsWithShades[$i][1][1],
				);
			}

			$ndf1 = pow($r - $this->colorsWithShades[$i][2][0], 2) + pow($g - $this->colorsWithShades[$i][2][1], 2) + pow($b - $this->colorsWithShades[$i][2][2], 2);
			$ndf2 = pow($h - $this->colorsWithShades[$i][3][0], 2) + pow($s - $this->colorsWithShades[$i][3][1], 2) + pow($l - $this->colorsWithShades[$i][3][2], 2);
			$ndf = $ndf1 + $ndf2 * 2;
		...
		if ($cl < 0) {
			...
			echo $this->colorsWithShades[$cl][0];

			return array(
				"hex" => "#" . $this->colorsWithShades[$cl][0],
				"name" => $this->colorsWithShades[$cl][1][0],
				"shade_name" => $this->colorsWithShades[$cl][1][1],
			);
		}
	}
